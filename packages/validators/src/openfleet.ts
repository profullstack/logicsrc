...
type FleetReferences = {
  ...
  rentals?: Array<{
    id: string;
    scope: { kind: "fleet" } | { kind: "members"; members: MemberReference[] };
...
export function validateOpenFleetReferences(data: unknown): ErrorObject[] {
  const fleet = data as FleetReferences;
  const errors: ErrorObject[] = [];
  function report(keyword: string, instancePath: string, message: string) {
    errors.push({ keyword, instancePath, schemaPath: "#/openfleet-semantics", params: {}, message });
  }

  const members = new Set<string>();
  fleet.members.forEach((member, i) => {
  ...
  fleet.rentals?.forEach((rental, i) => {
    ...
    if (rental.scope.kind === "members") {
      ...
      rental.scope.members.forEach((member, j) => {
        ...
        if (!members.has(key)) {
          report("memberReference", memberPath, "must reference a member of this fleet by kind and id");
        }
